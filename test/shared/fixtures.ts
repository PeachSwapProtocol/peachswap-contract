...
import PeachSwapFactory from '../../build/PeachSwapFactory.json'
import IPeachSwapPair from '../../build/IPeachSwapPair.json'
import PeachSwapRouter from '../../build/PeachSwapRouter.json'
...
export async function factoryFixture(_: Web3Provider, [wallet]: Wallet[]): Promise<FactoryFixture> {
  const factory = await deployContract(wallet, PeachSwapFactory, [wallet.address], overrides)
  return { factory }
}
...
export async function pairFixture(provider: Web3Provider, [wallet]: Wallet[]): Promise<PairFixture> {
  ...
  const pair = new Contract(pairAddress, JSON.stringify(IPeachSwapPair.abi), provider).connect(wallet)
  ...
  const WBNBPair = new Contract(WBNBPairAddress, JSON.stringify(IPeachSwapPair.abi), provider).connect(wallet)
...
export async function routerFixture(provider: Web3Provider, [wallet]: Wallet[]): Promise<RouterFixture> {
  ...
  const router = await deployContract(wallet, PeachSwapRouter, [factory.address, WBNB.address], overrides)
